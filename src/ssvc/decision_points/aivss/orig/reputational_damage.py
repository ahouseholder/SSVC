...
MINIMIZED_RISK = DecisionPointValue(
    name="Minimized Risk",
    key="M",
    definition="System designed to minimize reputational risks, with ongoing monitoring, stakeholder engagement, and a crisis management plan.",
)

RISK_ASSESSMENT = DecisionPointValue(
    name="Risk Assessment",
    key="R",
    definition="Reputational risk assessment conducted, with communication plans and mitigation strategies in place.",
)

AWARENESS = DecisionPointValue(
    name="Awareness",
    key="A",
    definition="Some awareness of reputational risks, limited monitoring of public perception, but no proactive measures.",
)

HIGH_RISK = DecisionPointValue(
    name="High Risk",
    key="H",
    definition="High risk of severe reputational damage due to system errors, biases, or security breaches, no mitigation strategies.",
)
...
REPUTATIONAL_DAMAGE = AivssDecisionPoint(
    ...
    key="RD",
    version="0.1.0",
    definition="Degree to which the system and organization mitigate reputational risks and manage public perception.",
